...
def create_browser_branch(source_branch):
    """Create a browser version of a branch by copying it."""
    print(f"\nCreating {source_branch}-browser branch...")
    
    # Switch to the source branch
    run_git_command(f"git checkout {source_branch}")
    
    # Create browser branch from current branch
    run_git_command(f"git checkout -b {source_branch}-browser")
    
    print(f"{source_branch}-browser branch created.")
...
def main():
    ...
    if "feature-power" in branches or "feature-modulo" in branches or "feature-power-browser" in branches or "feature-modulo-browser" in branches or "main-browser" in branches:
        response = input("\nBranches already exist. Delete and recreate? (y/n): ")
        if response.lower() == 'y':
            run_git_command("git checkout main")
            run_git_command("git branch -D feature-power feature-modulo feature-power-browser feature-modulo-browser main-browser 2>nul || git branch -D feature-power feature-modulo feature-power-browser feature-modulo-browser main-browser")
        else:
    ...
    # Create browser versions of all branches
    create_browser_branch("main")
    create_browser_branch("feature-power")
    create_browser_branch("feature-modulo")
    ...
    print("\nBrowser branches created:")
    print("- main-browser")
    print("- feature-power-browser")
    print("- feature-modulo-browser")
